...
DEVOICED_W = 0.55      # formed, barely voiced
PEAK = 0.45            # where in the vowel the opening is widest
...
def track_from_query(query, intensity=1.0, fps=30, blink=False, blink_seed=0):
    ...
    t = float(query.get("prePhonemeLength") or 0.0) / speed
    ...
    for cons, clen, vowel, vlen in ms:
        ...
        if v in ("pau", "cl", "N", "n"):
            ...
            ev.append((t, None, 0.0))
            ev.append((t + vlen * 0.9, None, 0.0))
        elif v.lower() in V_OF:
            w = intensity * (DEVOICED_W if v.isupper() else 1.0)
            ev.append((t + vlen * PEAK, V_OF[v.lower()], round(w, 3)))
        t += vlen
    t += float(query.get("postPhonemeLength") or 0.0) / speed
    ev.append((t, None, 0.0))
